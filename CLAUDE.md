...
- MiMo TokenPlan API（mimo-v2.5-pro）
...
- 核心引擎仅使用 Python 标准库
- LLM 集成层允许 `openai` SDK（与 Flask 例外一致）
- 代码风格与现有模块一致
...
- 客户端：`src/llm/client.py`（OpenAI 兼容 SDK 封装）
- 配置：`src/llm/config.py`（API key 从 `config/llm.json` 或环境变量读取）
- Prompt：`src/llm/prompts.py`（5 个中文 task prompt）
- 成本追踪：`src/llm/cost_tracker.py`（$20 月度熔断）
...
├── corpus/              # 语料数据
│   └── refinements/     # 标注叠加层（待实现）
├── prompts/             # Prompt 模板
│   └── ngp-skills/      # NGP Skills
├── config/              # 配置文件
│   └── llm.json         # LLM API 配置（gitignored）
├── scripts/             # 测试和工具脚本
├── projects/            # 用户项目数据
└── tests/               # 测试
```
